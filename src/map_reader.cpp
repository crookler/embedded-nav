...
void visualizeTrajectory(const OccupancyGrid& true_grid,
    ...
    // Draw the grid / inflated obstacles
    for (int row = 0; row < true_grid.rows(); ++row) {
        for (int column = 0; column < true_grid.columns(); ++column) {
            if (true_grid.getCell(row, column) >= OBSTACLE_THRESHOLD) {
                const double x = origin_x + column * resolution;
                const double y = origin_y + row * resolution;
    ...
    // i hate this.
    // legend();
    title("A* Path with LQR Tracking");
    xlim({origin_x, origin_x + true_grid.columns() * resolution});
    ylim({origin_y, origin_y + true_grid.rows() * resolution});
